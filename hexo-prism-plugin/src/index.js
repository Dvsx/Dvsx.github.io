...
const map = {
  ...
  '&amp;': '&',
  '&gt;': '>',
  '&lt;': '<', '&quot;':="" '"'="" };="" const="" themeregex="/^prism-(.*).css$/;" regex="/<pre"><code class="(.*)?">([\s\S]*?)<\ code=""><\ pre="">/igm;
...
function PrismPlugin(data) {
  ...
  if (captionRegex.test(data.content)) {
    ...
    data.content = data.content.replace(captionRegex, (origin, lang, caption, code) => {
      ...
      return `<figcaption>${caption}</figcaption><pre class=" language-${lang}"><code class="language-${lang}">${code}</code></pre>`;
    })
  }
